-- SQL Server Schema for AI Management System

-- DROP TABLES (MSSQL syntax for dropping tables if they exist)
...
-- Create tables in proper order to avoid foreign key constraint issues
...
CREATE TABLE AI_USER (
    ...
    CONSTRAINT FK_AI_USER_ROLE_ID FOREIGN KEY (ROLE_ID) REFERENCES AI_USER_ROLE(ROLE_ID)
);
...
CREATE TABLE AI_MAP_USER_GROUP_PERMISSION (
    ...
    CONSTRAINT PK_AI_MAP_USER_GROUP_PERMISSION PRIMARY KEY (USER_ID, GROUP_ID),
...
CREATE TABLE DEVICE_SET_ACTIONS (
    ...
    DEVICE_ID       VARCHAR(200) NOT NULL,
    CONSTRAINT PK_DEVICE_SET_ACTIONS PRIMARY KEY (COMMAND, DEVICE_ID, REQUEST_VALUE),
...
-- Using VARCHAR(MAX) for array-like storage
...
-- Adding missing tables that were in the DROP section but not in CREATE section
...
CREATE TABLE MESSAGE_ALERT_HISTORY (
    ...
    CREATED_DATE      DATETIME DEFAULT GETDATE(),
    TEMPLATE_ID       INT,
    CONSTRAINT FK_MESSAGE_ALERT_HISTORY_TEMPLATE_ID FOREIGN KEY (TEMPLATE_ID) REFERENCES MESSAGE_ALERT_TEMPLATE(TEMPLATE_ID)
);

CREATE TABLE MESSAGE_ALERT_HISTORY_MAP_DEVICE (
    ALERT_ID          INT NOT NULL,
    DEVICE_ID         VARCHAR(200) NOT NULL,
    CONSTRAINT PK_MESSAGE_ALERT_HISTORY_MAP_DEVICE PRIMARY KEY (ALERT_ID, DEVICE_ID),
    CONSTRAINT FK_MESSAGE_ALERT_HISTORY_MAP_DEVICE_ALERT_ID FOREIGN KEY (ALERT_ID) REFERENCES MESSAGE_ALERT_HISTORY(ALERT_ID) ON DELETE CASCADE,
    CONSTRAINT FK_MESSAGE_ALERT_HISTORY_MAP_DEVICE_DEVICE_ID FOREIGN KEY (DEVICE_ID) REFERENCES DEVICE_INFO(DEVICE_ID) ON DELETE CASCADE
);

CREATE TABLE UPLOADED_FIRMWARE_INFO (
    FIRMWARE_ID       INT IDENTITY(1,1) PRIMARY KEY,
    FIRMWARE_NAME     VARCHAR(200),
    FIRMWARE_VERSION  VARCHAR(60),
    FIRMWARE_PATH     VARCHAR(1000),
...
CREATE TABLE AI_SCHEDULE_TASK (
    ...
    SCHEDULE_TIME     VARCHAR(100),  -- Cron expression or similar
